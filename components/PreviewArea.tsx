  ...
  return (
    <div className="w-full h-full flex flex-col">
      ...
      {currentResult && !isGenerating && (
        <div className="mt-6 p-4 rounded-xl border border-zinc-800 bg-zinc-900/50 backdrop-blur-sm">
           ...
           <p className="text-xs text-gray-500 font-mono line-clamp-3 mb-4 opacity-70">
           ...
           <div className="flex gap-3">
              <a 
                ...
                className="flex-1 bg-white text-black text-center py-2.5 rounded-lg text-sm font-semibold hover:bg-gray-100 transition-colors"
              >
                Download PNG
              </a>
              {/* Generate Vertical Button (Only visible if not already 9:16) */}
              {currentResult.settings.aspectRatio !== '9:16' && (
                 <button 
                   onClick={onGenerateMobile}
                   className="flex-1 bg-zinc-800 text-white border border-zinc-700 hover:bg-zinc-700 py-2.5 rounded-lg text-sm font-semibold transition-colors flex items-center justify-center gap-2 group"
                 >
                   <svg className="w-4 h-4 text-gray-400 group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
                   Generate Vertical
                 </button>
              )}
           </div>
